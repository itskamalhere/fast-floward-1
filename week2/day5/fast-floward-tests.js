...
describe('Flow Dapp Tests', async () => {
    ...
    describe('Fast Floward Tests', async () => {
        ...
        it(`removes a kittyitem from sale`, async () => {
            ...
            assert.equal(res1.result, 1, "The Sale Collection length is not 1")


            // 8) assert the result (an array) from step 6) shows the SaleCollection has 
            // an itemID == 1 in it at index 0
            assert.equal(res2.result[0], 1, "Index 0 of Sale Collection IDs is not 1")
        })
        ...
        it(`checks both admin and user have correct balances after purchase`, async () => {
            ...
            assert.equal(res1.result, 5.0, "The account address " + config.accounts[0] + "doesn't have 5.0 Kibble")

            // 6) assert the result from step 4) equals 25.0
            assert.equal(res2.result, 25.0, "The account address " + config.accounts[1] + "doesn't have 25.0 Kibble")
        })

        it(`burn kibble tokens from the account and has the correct balance in the account`, async () => {
            let testData1 = {
                signer: config.accounts[1],
                amount: "15.0"                
            }
            ...
            assert.equal(res1.result, 10.0, "Account has incorrect balance after burn")

            assert.equal(res2.result, 15.0, "Incorrect total number of Kibble after burn")
        })        
    });
